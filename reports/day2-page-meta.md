...
post: the chain-census check now lives in the Limits, so its table is the
...
### `images/01-growth.png` (1161×687) — growth by format
...
### `images/02-retention.png` (1461×823) — cohort retention
...
### `images/03-dollar-only-zips.png` (1158×687) — dollar-only ZIP codes
...
### `images/04-dollar-only-split.png` (1618×434) — how the ZIP codes joined
...
- The chain-census table sits in the Limits: it shows authorization counts
  track store counts at the end of the window (and, per the 2006 check, not
  at the start). The body speaks only of authorizations.
